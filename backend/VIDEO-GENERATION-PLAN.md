...
take + sport + tone + seconds
        │
0. Enhance + ask               sharpen take · pick style preset · ask the gaps   <$0.01
...
6. Keyframe per scene          Grok image (≤3 escalating attempts) + vision gate $0.05–0.15 each
...
**Stage 0 — enhancement and input collection** (`enhancer.py`)

Quality is decided before money is spent. One LLM call sharpens the take
(keeping the stance), extracts subjects and teams, and flags genuine
ambiguity. The stage then returns `Question` objects for gaps that measurably
change the output — thin take, unknown cast, ambiguous team, unstated
tone/length/look — each with a default, so an unanswered question is never a
blocker. Answers fold back in via `apply_answers()`.

**Style is a fixed preset, never model-authored.** Three photographic looks:
`broadcast` (clean television), `cinematic` (golden hour), `gritty`
(documentary). A free-form style line per job is not repeatable, and letting
one exist caused the cartoon-scene defect described in §9.
...
is_photoreal is true ONLY for what could plausibly be a real photograph:
real skin texture, fabric weave, optical depth of field. False for cartoons,
anime, renders, digital paintings and cel-shaded art. A stylised grade on a
real photo is still photoreal; a drawing never is.
```

**Hard fail (regenerate):** text defect · real logo · collage · **not photoreal** · severe anatomy.  
**Soft warn (continue):** odd hands · minor anatomy · busy lower quarter · subject doubt.  
`subject_matches` measured noisy in both directions → warning only.

**Retry policy:** up to 3 attempts, each escalated with a correction aimed at
the reported defect. If none pass, the attempt with the fewest hard failures
ships and the run records a warning.
...
9. **The photoreal anchor is non-overridable.** `PHOTOREAL` opens and closes
   every image and motion prompt. Job style may only *add* grade and mood on
   top of `STYLE_BIBLE`, never replace it, and medium words are stripped from
   anything a model writes.
   *Measured:* `style_for()` once read `plan.style or STYLE_BIBLE`, so a
   model-authored style deleted the only mention of photorealism. Scene 2 of a
   Messi/Ronaldo run came back as an isometric cartoon — inside an otherwise
   photoreal video — and both retries failed. Cost: $1.23, unusable scene.
10. **No text-bearing props in any scene.** Newspapers, phones, tablets,
    screens, signs, whiteboards, documents. The image model renders lettering
    as garbled nonsense and the gate rejects the frame. Wordless physical
    props only. Any that slip through are explicitly blanked in the prompt.
11. **Retries escalate.** A rejected keyframe is regenerated with a correction
    targeting the specific defect the gate reported, up to 3 attempts. If none
    pass, the attempt with the *fewest* hard failures ships, and the run is
    warned. Resending an identical prompt reproduces the identical defect.
