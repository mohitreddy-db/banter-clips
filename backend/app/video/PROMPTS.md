...
|---|---|---|---|---|
| `take_enhancer` | system | input page, on demand and repeatable | `OPENAI_PLAN_MODEL` (gpt-4.1) | **Instagram Sports Banter specialist.** Two sharper takes in *different angles* per press. Optimises for what spreads on Reels: front-loaded hook, a visual situation, enough disagreement to pull replies. Never reverses the stance; the original always stays selectable. |
| `captions` | system | publishing, on demand | `OPENAI_PLAN_MODEL` | Three Instagram captions in different registers with specific hashtags. |
| `enhancer` | system | `enhance` (API only; the UI no longer calls it) | `OPENAI_PLAN_MODEL` | Sharpens a take and reports subjects/teams/ambiguity. Superseded in the UI by `take_enhancer`. |
| `planner` | system | `planning_story` | `OPENAI_PLAN_MODEL` (gpt-4.1) | **Instagram Sports Banter Reels specialist.** Take → shot-by-shot JSON plan directing camera (four axes), blocking, expression, lighting, quantified beats, diegetic sound and transitions. Encodes the load-bearing rules: keep the stance; one speaker per scene, consecutive scenes differ (voice drift); one camera setup per scene (collage defence); no text-bearing props; live action only; no invented facts. |
